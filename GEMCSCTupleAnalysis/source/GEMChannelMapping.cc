...
#include <sstream>
#include<regex>
#include "../include/GEMChannelMapping.h"
#include "../include/GEMConfigInfo.h"
...
GEMChannelMapping::GEMChannelMapping(const GEMConfigInfo& gemConfig, int nPartitions, int nPhis){
  getStripMap(gemConfig,nPartitions,nPhis);
  getGEBSlotMap(gemConfig,nPartitions,nPhis);
}
...
void GEMChannelMapping::getGEBSlotMap(const GEMConfigInfo& gemConfig, int nPartitions, int nPhis){
  gebSlotMap.clear();
  gebSlotMap.resize(nPartitions*nPhis);
  std::ifstream ifile;
  ...
  for (int row = 0; row < nPhis; row++) {
    std::string line;
    std::getline(ifile, line);
    std::istringstream iss(line);
    if ( !ifile.good() ) break;
    for (int col = 0; col < nPartitions; col++) {
      std::string val;
      std::getline(iss,val,',');
      std::stringstream convertor(val);
      convertor >> std::hex >> gebSlotMap[nPartitions*row+col];
    }
  }
  ifile.close();
}

void GEMChannelMapping::getStripMap(const GEMConfigInfo& gemConfig, int nPartitions, int nPhis){
  vFatStripMap.clear();
  vFatStripMap.resize(nPartitions*nPhis);
  for (int iV = 0; iV < nPartitions*nPhis; iV++){
