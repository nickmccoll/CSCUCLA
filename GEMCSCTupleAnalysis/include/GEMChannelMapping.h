...
public:
  GEMChannelMapping(const GEMConfigInfo& gemConfig, int nPartitions, int nPhis);
...
private:
  ...
  void getGEBSlotMap(const GEMConfigInfo& gemConfig, int nPartitions, int nPhis);
  //Reads map of per vfat to physical location
  void getStripMap(const GEMConfigInfo& gemConfig, int nPartitions, int nPhis);
};
}
